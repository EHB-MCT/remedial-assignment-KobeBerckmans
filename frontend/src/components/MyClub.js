...
function MyClub({ user, club, onClubUpdate }) {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        try {
          ...
          setMessage(`Player listed for sale but failed to update club. Please refresh the page.`);
        }
      }
    } catch (error) {
  ...
  return (
    <div className="my-club">
      ...
      {message && (
        <div className={`message ${message.includes('sold') || message.includes('listed') ? 'success' : 'error'}`}>
          {message}
        </div>
      )}
      ...
      {showSellModal && selectedPlayer && (
        <div className="sell-modal-overlay">
          <div className="sell-modal">
            <h3>Sell {selectedPlayer.name}</h3>
